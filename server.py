# SGCD v2.47.1 — Servidor local: SQLite, autenticação, REST API, proxy CNPJ, e-mail SMTP, backup automático
import http.server
import os
import json
import sqlite3
...
_log = sgx_base.configurar_log('SGCD', _DATA_DIR)
...
class SGCDHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def _safe_dispatch(self, inner):
        ...
        try:
            ...
            # classifica: ErroCliente -> 400 (WARNING sem stack); resto -> 500 (ERROR + traceback)
    ...
    def _do_POST(self):
        ...
        # Erro de JavaScript reportado pelo navegador. Sem auth (o erro pode ser
        # antes do login) e throttled no motor — nunca deixa um browser em loop
        # afogar o log. Responde sempre 204, para o report ser fire-and-forget.
        if p == '/api/log/client':
            try:
    ...
    def _route_get(self, p, qs, s):
        ...
        # Arquivos
            ...
            pid    = qp('process_id')
            prefix = qp('prefix') == '1'
            per    = sgx_base.int_param(qs, 'per', 200, minimo=1, maximo=1000)
            with get_db() as conn:
                if pid and prefix:
        ...
        # Usuários (admin)
            ...
            if not s['admin']: self._json(403, {'error': 'Acesso restrito'}); return
            self._json(200, sgx_base.ler_diagnostico_erros(_DATA_DIR, 'SGCD'))
        ...
        # Backup
            ...
            tmpdir = _tf.mkdtemp()
            try:
                name = time.strftime('DB_SGCD_BACKUP_%Y-%m-%d_%H-%M-%S.zip')
            ...
            bdir = cfg['path']
            files = sorted(
                (f for f in os.listdir(bdir) if f.startswith('DB_SGCD_BACKUP_') and f.endswith(_COFRE_EXTS)),
                reverse=True
            ...
            if not name or not name.startswith('DB_SGCD_BACKUP_') or not name.endswith(_COFRE_EXTS) or '/' in name or '\\' in name:
    ...
    # Exporta o cadastro canônico e importa o de outro sistema por CNPJ, com
    # last-write-wins e revisão manual do conflito (ver sgx_base.classificar_merge).
    # Preview não escreve; apply escreve. syncedAt = marca d'água do último sync.
    def _export_fornecedores(self):
        ...
        for r in rows:
            ...
            if not sgx_base.cnpj_digits(d.get('cnpj')):
                continue   # sem CNPJ não sincroniza (não há chave)
            itens.append({c: d[c] for c in sgx_base.CAMPOS_FORNECEDOR if c in d})
        payload = {'_sgx': 'SGCD', 'tipo': 'fornecedores', 'exportedAt': _now(), 'fornecedores': itens}
    ...
    def _forn_entrada_valida(self, data):
        # Aceita arquivo de qualquer sistema da família (peer) — só exige a forma.
    ...
    def _forn_upsert(self, conn, local, item):
        """Grava um fornecedor vindo do sync. Ao atualizar um existente, preserva
        os vínculos LOCAIS do SGCD (processos, certidões) — só o SGCD os tem."""
        canon = {c: item[c] for c in sgx_base.CAMPOS_FORNECEDOR if c in item}
        if local:
            fid = local['_id']
            base = {k: v for k, v in local.items() if k != '_id'}
            merged = {**base, **canon,
                      'processos': base.get('processos', []),
                      'certidoes': base.get('certidoes', [])}
        else:
            fid = str(uuid.uuid4())
            merged = dict(canon)
        merged['id'] = fid
        merged['cnpj_digits'] = sgx_base.cnpj_digits(merged.get('cnpj'))
    ...
    def _forn_marcar_synced(self, conn, local, ate):
        """Conflito resolvido a favor do local: mantém o dado, mas avança a marca
        d'água pra o do arquivo não reaparecer como conflito no próximo sync."""
        fid = local['_id']
    ...
    def _sync_forn_apply(self, data):
        ...
        if entrada is None:
            self._json(400, {'error': 'Arquivo não é um cadastro de fornecedores válido'}); return
        resolver = data.get('resolver') or {}   # {cnpj: 'local' | 'arquivo'}
        with get_db() as conn:
            ...
            for c in plano['conflitos']:
                ...
                elif local:   # 'local' ou sem escolha: mantém o local, avança a marca
    ...
    def _export_backup(self):
        ...
        self.send_header('Content-Disposition',
                         f'attachment; filename="SYNC_SGCD_BACKUP_{time.strftime("%Y-%m-%d_%H-%M-%S")}.json"')
        self.end_headers()
...
# sgx_base. Flag de usuários = False (SGCD é de admin único; contas não viajam no
# JSON portátil — para clonar login usa-se o Cofre). Ver sgx_base.eh_backup.
_SGX_SIGLA = 'SGCD'
_BACKUP_SCHEMA = 5
_BACKUP_INCLUI_USUARIOS = False
_COFRE_EXTS = ('.zip', '.db')   # casa o .zip novo e o .db legado (rotação/listagem)
...
_SYNC_PREFIXOS = ('SYNC_SGCD_BACKUP_', 'SIS_SGCD_BACKUP_')
...
def _do_json_backup(cfg=None):
    ...
    name = time.strftime('SYNC_SGCD_BACKUP_%Y-%m-%d_%H-%M-%S.json')
...
def _rotate_backups(cfg=None):
    ...
    for prefix, ext in [('DB_SGCD_BACKUP_', _COFRE_EXTS), (_SYNC_PREFIXOS, '.json')]:
...
if __name__ == '__main__':
    _selecionar_modo()
    sgx_base.instalar_captura_de_falhas(_DATA_DIR, 'SGCD')  # crash log que sobrevive à janela fechar
    _check_db_integrity()
    ...
    browser = None if os.environ.get('SGCD_NO_BROWSER') else _find_browser()
    if browser:
